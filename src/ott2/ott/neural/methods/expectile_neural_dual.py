...
from ott2.neural.networks.potentials import (
    BasePotential,
    PotentialGradientFn_t,
    PotentialTrainState,
    PotentialValueFn_t,
)
from ott2.problems.linear.potentials import DualPotentials
...
def restore_optimizer_state(opt_state: optax.OptState, restored) -> optax.OptState:
...
class PotentialTrainStateWithStats(PotentialTrainState):
  batch_stats: FrozenDict[str, jnp.ndarray]
...
class PotentialModelWrapper(BasePotential):
  ...
  def __call__(self, x: jnp.ndarray, train=True) -> jnp.ndarray:
    ...
    z: jnp.ndarray = self.model(x, train)
  ...
  def potential_value_fn(
    ...
    if batch_stats is not None:
      return lambda x: self.apply({"params": params, 'batch_stats': batch_stats}, x, train=True, mutable=['batch_stats'])
    return lambda x: self.apply({"params": params}, x)

    
  def potential_gradient_fn(
      self, params: FrozenDict[str, jnp.ndarray], batch_stats,
  ) -> PotentialGradientFn_t:
    """A vector function or gradient of the potential."""
    # if self.is_potential:
    #   if batch_stats is not None:
    #     def fn(x):
    #       v, updates = self.potential_value_fn(params, batch_stats)(x)
    #       return jax.grad(v), updates
    #     return fn
    #   return jax.grad(self.potential_value_fn(params, None))
  ...
  def create_train_state(
      ...
      **kwargs: Any,
  ) -> PotentialTrainStateWithStats:
    """Create initial training state."""
    variables = self.init(rng, jnp.ones(input))
    params = variables['params']

    return PotentialTrainStateWithStats.create(
        apply_fn=self.apply,
        params=params,
        tx=optimizer,
...
class ExpectileNeuralDual:
  ...
  def __init__(
    ...
    if start_step > 0:
      ...
      ckp_f = ckptr.restore(f"/home/nazar/projects/ott/src/notebooks/outputs2/{start_step}/state_f")
      ckp_g = ckptr.restore(f"/home/nazar/projects/ott/src/notebooks/outputs2/{start_step}/state_g")
  ...
  def train_fn(
    ...
    for step in tqdm(range(self.start_step + 1, self.num_train_iters + self.start_step + 1)):
      ...
      if step % 10000 == 3:
        ckptr = PyTreeCheckpointer()
        ckptr.save(
            f"/home/nazar/projects/ott/src/notebooks/outputs2/{step}/state_f",
            self.state_f,
            force=True,
            save_args=orbax_utils.save_args_from_target(self.state_f),
        )
        
        ckptr = PyTreeCheckpointer()
        ckptr.save(
            f"/home/nazar/projects/ott/src/notebooks/outputs2/{step}/state_g",
            self.state_g,
            force=True,
            save_args=orbax_utils.save_args_from_target(self.state_g),
        )
  ...
  def _get_train_step(
    ...
    def step_fn(state_f, state_g, batch):
      ...
      (loss, (updates_f, loss_f, loss_g, w_dist)), (grads_f, grads_g) = grad_fn(
          state_f.params,
          state_g.params,
          state_f.batch_stats, state_g.batch_stats,
          state_f.potential_gradient_fn,
          state_g.potential_value_fn,
          batch,
      )

      state_f = state_f.apply_gradients(grads=grads_f)
      state_f = state_f.replace(batch_stats=updates_f['batch_stats'])
  ...
  def _get_valid_step(
    ...
    def step_fn(state_f, state_g, batch):
      grad_fn = jax.value_and_grad(self._loss_fn, argnums=[0, 1], has_aux=True)
      (loss, (updates_f, loss_f, loss_g, w_dist)), _ = grad_fn(
          state_f.params,
          state_g.params,
          state_f.batch_stats, state_g.batch_stats,
          state_f.potential_gradient_fn,
  ...
  def _get_g_value_partial(
      self, params_g: FrozenDict[str, jnp.ndarray], stats_g,
      g_value: Callable[[FrozenDict[str, jnp.ndarray]], PotentialValueFn_t]
  ):

    if self.use_dot_product:
      g_value_partial = lambda y: -jax.vmap(g_value(params_g, stats_g))(y)
      g_value_partial_detach = \
          lambda y: -jax.vmap(g_value(stop_gradient(params_g), stats_g))(y)
    else:
      g_value_partial = g_value(params_g, stats_g)
      g_value_partial_detach = g_value(stop_gradient(params_g), stats_g)
  ...
  def _loss_fn(
    ...
    g_value_partial, g_value_partial_detach = self._get_g_value_partial(
        params_g, stats_g, g_value
    )
    batch_cost = self.train_batch_cost

    # transport = ENOTPotentials(
    #     gradient_f(params_f, stats_f), g_value(params_g, stats_g), self.cost_fn,
    #     self.is_bidirectional, self.use_dot_product
    # ).transport

    if self.is_bidirectional and not self.use_dot_product:
      transport = lambda x, g: self.cost_fn.twist_operator(x, g, False)
    else:
      transport = lambda x, g: g

    grad_f_source, updates_f = gradient_f(params_f, stats_f)(source)

    target_hat = jax.vmap(transport, in_axes=(0, 0))(
      source, 
      grad_f_source
    )
    ...
    return loss, (updates_f, f_loss, g_loss, w_dist)

  def to_dual_potentials(self) -> DualPotentials:
    """Return the Kantorovich dual potentials from the trained potentials."""
    # f_grad_partial = self.state_f.potential_gradient_fn(self.state_f.params, self.state_f.batch_stats)
    f_grad_partial = lambda x: self.state_f.apply_fn({"params": self.state_f.params, 'batch_stats': self.state_f.batch_stats}, x, train=False, mutable=False)
    g_value_partial = self.state_g.potential_value_fn(self.state_g.params, self.state_g.batch_stats)
