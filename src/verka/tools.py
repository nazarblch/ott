        ...
        with torch.no_grad():
            for step, (X, _) in tqdm(enumerate(loader)):
                for i in range(0, len(X), batch_size):
                    ...
                    # batch = F.upsample(batch, size=(img_size//2, img_size//2), mode='bilinear')
                    # x_sub = F.upsample(X[start:end], size=(img_size//2, img_size//2), mode='bilinear')
                    x_sub = X[start:end]
                    mse_arr.append(F.mse_loss(x_sub, batch).cpu().numpy() * (end - start))
                    batch = batch.to(device) * 0.5 + 0.5
                    pred_arr.append(model(batch)[0].cpu().data.numpy().reshape(end-start, -1))
...
def get_pushed_loader_stats_2(T, loader, inception_fn, inception_params, batch_size=8, n_epochs=1, verbose=False, device='cuda',
                            use_downloaded_weights=False, upgrade=False):
    dims = 2048
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    model = InceptionV3([block_idx], use_downloaded_weights=use_downloaded_weights).to(device)
